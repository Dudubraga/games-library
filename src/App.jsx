...
export default function App() {
  ...
  const filtered = useMemo(() => {
    let list = games.filter((g) => {
      ...
      if (filterCategory && !(g.categories || []).includes(filterCategory))
        return false;
    ...
    list = [...list].sort((a, b) => {
      switch (sortBy) {
        ...
        case "wishlist":
          return (
            (STATUS_ORDER[b.status] ?? 2) - (STATUS_ORDER[a.status] ?? 2) ||
            a.title.localeCompare(b.title, "pt-BR")
          );
        case "platform":
          return (
  ...
  const openRandom = () => {
    if (!games.length) {
      toast("Nenhum jogo na biblioteca");
      return;
    }
    setModal("random");
  };
  ...
  const openRoulette = () => {
    if (games.length < 2) {
      toast("Adicione pelo menos 2 jogos");
      return;
    }
    setModal("roulette");
  };
  ...
  return (
    <div className="app">
          ...
          <div className="filter-group">
            ...
            {PLATFORMS.map((p) => (
              <button
                ...
                onClick={() =>
                  setFilterPlatform((prev) => (prev === p.id ? null : p.id))
                }
              >
                {p.ico} {p.label}
              </button>
          ...
          <div className="filter-group">
            ...
            {STATUSES.map((s) => (
              <button
                ...
                onClick={() =>
                  setFilterStatus((prev) => (prev === s.id ? null : s.id))
                }
              >
                {s.ico} {s.label}
              </button>
      ...
      {modal === "roulette" && (
        <RouletteModal games={sortedGames} onClose={closeModal} />
      )}

      {modal === "import" && (
        <ImportModal onImport={handleBatchImport} onClose={closeModal} />
      )}
